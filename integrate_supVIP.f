c 21 Nov 2017 adapt for gfortran, Linux
! 8 March 2017, in son_of_groucho: take integrate_deepLTSx.f from deltaAIX and rename it
! (was) Integration program for superior & deep LTS cells
! From ltsi.f in supergj.f

       SUBROUTINE integrate_supVIP   (O, time, numcell, V, curr,
...
7000     CONTINUE
        ...
        CALL supVIPIMAJ (GL,GAM,GKDR,GKA,GKC,GKAHP,GK2,GKM,
...
c above assumes equil. potential for AMPA & NMDA = 0 mV
     ...
     &   (v(igap2,L1) - v(igap1,L)) * cinv(igap1)
       ...
       do i = 1, numcomp
        gamma(i) = min (1.d0, .004d0 * chi(i,L))
...
88     FV(I) = FV(I) - ( gna_tot(i) * (v(i,L) - vna)
     ...
     x       - betchi(i) * chi(i,L)
       ...
       do i = 1, numcomp
        alpham_ahp(i) = min(0.2d-4 * chi(i,L),0.01d0)
...
c      goto 4000
       ...
       K1 = nint ( 4.d0 * (V(I,L) + 120.d0) )
       IF (K1.GT.640) K1 = 640
       IF (K1.LT.  0) K1 =   0

             fastNa_shift = -2.5d0
       K0 = nint ( 4.d0 * (V(I,L)+   fastNa_shift+ 120.d0) )
       IF (K0.GT.640) K0 = 640
       IF (K0.LT.  0) K0 =   0
