c 23 Aug 2019, tuftRS not used in initial piriform.f
c so this program becomes a placeholder
c Need to make sure there is no false contribution to fields
...
c rectifier (ar).
c       SUBROUTINE integrate_tuftRSXXB (O, time, numcell, V, curr,
        SUBROUTINE integrate_placeholder4 (O, time, numcell, V, curr,
...
c this will be gCa conductance corresponding to high-thresh channels
       REAL*8 A, BB1, BB2
       real*8 depth(18), membcurr(18), field_sup, field_deep
...
! Add membrane currents into membcurr for appropriate compartments
         ...
         do i = 1, 18
c       field_sup = field_sup + membcurr(i) / dabs(100.d0 - depth(i))
        field_sup = 0.d0 
c       field_deep = field_deep + membcurr(i) / dabs(500.d0 - depth(i))
        field_deep = 0.d0 
         end do
